...
export const event = pgTable("event", {
	...
	createdDate: timestamp("createdDate", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedDate: timestamp("updatedDate", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
...
export const userRace = pgTable("user_race", {
	id: uuid("id").default(sql`uuid_generate_v4()`).primaryKey().notNull(),
	createdDate: timestamp("createdDate", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedDate: timestamp("updatedDate", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
...
(table) => {
	return {
		...
		uqE12875Dfb3B1D92D7D7C5377E22: unique("UQ_e12875dfb3b1d92d7d7c5377e22").on(table.email),
	}
});